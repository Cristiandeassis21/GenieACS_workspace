...
- **Tradutor de Interface LAN:** Injetada lógica de React no componente \`device-page-tr181.jsx\` para traduzir o nome bruto das interfaces LAN (ex: \`Device.Ethernet.Interface.4.\`) para nomenclaturas amigáveis ("Cabo de Rede" e "Wi-Fi") na aba de Hosts Conectados.
...
- **Refatoração de Mapeamento TP-Link XX230v:** Alterado o provision script \`default.js\` e a interface \`device-page-tr181.jsx\` para mapear as folhas exatas não padronizadas da TP-Link, como \`OpticalSignalLevel\` no lugar de \`ReceiveOpticalPower\` e a mudança do IPv6 da LAN para o índice 7.
...
- **Renderização Undefined em Cartões:** Corrigido o filtro de \`undefined\` e o mapeamento de \`LAN Hosts\` que causava a exibição de strings em branco. O script de provisão agora requisita nomes de folhas exatos (\`IPAddress\` ao invés do padrão \`IPv4Address\`) para popular as tabelas.
